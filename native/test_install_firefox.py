...
class FirefoxInstallTest(unittest.TestCase):
    def test_registration_matches_extension_and_preserves_chromium(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            extension = json.loads((Path(__file__).parents[1] / 'extension/manifest.json').read_text())
            self.assertEqual(manifest['allowed_extensions'], [extension['browser_specific_settings']['gecko']['id']])
